...
public class EtapaDosCurso extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        btnContinuar.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                boolean testApproved = sharedPreferences.getBoolean(TEST_APPROVED_KEY2, false);
                int percentage = sharedPreferences.getInt("TestPercentage2", 0);

                if (testApproved && percentage >= 80) {
                    // Navigate to EtapaTresCurso
                    EtapaTresCurso EtapaTresCurso = new EtapaTresCurso();
                    requireActivity().getSupportFragmentManager().beginTransaction()
                            .setCustomAnimations(R.anim.slide_in_right, R.anim.slide_out_left, R.anim.slide_in_left, R.anim.slide_out_right)
                            .replace(R.id.main_container, EtapaTresCurso)
                            .addToBackStack(null)
                            .commit();
                } else {
                    // Show dialog to complete Test 2 first
    ...
    private void showTestCompletionDialog() {
        ...
        builder.setTitle("Completar Test 2")
                .setMessage("Debes completar el Test 2 (Phishing por correo electronico) antes de continuar.")
